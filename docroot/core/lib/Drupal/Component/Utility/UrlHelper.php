...
class UrlHelper {
  ...
  public static function parse($url) {
    ...
    if ($scheme_delimiter_position !== FALSE && ($query_delimiter_position === FALSE || $scheme_delimiter_position < $query_delimiter_position) && ($fragment_delimiter_position === FALSE || $scheme_delimiter_position < $fragment_delimiter_position)) {
      ...
      if (isset($parts[1])) {
        $options['query'] = static::parseString($parts[1]);
      }
    }
    // Internal URLs.
    else {
      ...
      if (isset($parts['query'])) {
        $options['query'] = static::parseString($parts['query']);
      }
  ...
  /**
   * An alternative to parse_str() that handles repeated parameters.
   *
   * @param string $string
   *   A query string.
   *
   * @return array
   *   An array representation of the given string.
   *
  ...
  public static function parseString($string) {
    ...
    foreach ($chunks as $delta => $chunk) {
      ...
      // Get the name of this part of the query string. Examples:
      // - a[key]=1 -> a
      // - a[key]   -> a
      // - a[]=1    -> a
      // - a[]      -> a
      ...
      // Check if this part of the query uses the array syntax. Example matches:
      // a[key]=1, a[key], a[]=1, a[].
      $usesArraySyntax = preg_match('/\[.*\][=$]/', $chunk);

      if (!in_array($keyName, $keys)) {
        // This is the first time this key is spotted.
        $keys[] = $keyName;
      }
      elseif (!$usesArraySyntax) {
        // This param is not a php-style array and it has already occurred
        // in the query string. Take it out for separate processing so it's not
        // overridden by parse_str().
        unset($chunks[$delta]);
        $chunksForIndividualProcessing[] = $chunk;
      }
    }

    if (!empty($chunks)) {
      // By now $chunks should only have pieces that won't cause any data loss.
      parse_str(implode('&', $chunks), $result);
    }

    if (!empty($chunksForIndividualProcessing)) {
      foreach ($chunksForIndividualProcessing as $individualChunk) {
        $individualProcessingResult = [];
        parse_str($individualChunk, $individualProcessingResult);
        foreach ($individualProcessingResult as $key => $value) {
          if (!isset($result[$key])) {
            $result[$key] = $value;
          }
