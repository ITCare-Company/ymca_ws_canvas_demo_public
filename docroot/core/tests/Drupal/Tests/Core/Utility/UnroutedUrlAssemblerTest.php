...
class UnroutedUrlAssemblerTest extends UnitTestCase {
   ...
   */
  public function testAssembleWithExternalUrlWithoutQueryFragmentOrHttps() {
    $unrouted_url_assembler = $this->getMockBuilder(UnroutedUrlAssembler::class)
      ->setConstructorArgs([$this->requestStack, $this->pathProcessor])
      ->setMethods(['addOptionDefaults'])
      ->getMock();
    // Make sure that the method addOptionDefaults() does not get called.
    $unrouted_url_assembler->expects($this->never())
      ->method('addOptionDefaults');

    // Gain access to the protected method.
    $method = new \ReflectionMethod($unrouted_url_assembler, 'addOptionDefaults');
    $method->setAccessible(TRUE);

    $uri = 'http://example.com/test';
    $this->assertEquals($uri, $unrouted_url_assembler->assemble($uri, [], FALSE));
  }
