...
import { useIsDropAllowed } from '@/hooks/useIsDropAllowed';
...
export interface ComponentDropZoneProps {
  ...
  slotOwnerComponent?: ComponentNode; // The component that owns the parentSlot
}
const ComponentDropZone: React.FC<ComponentDropZoneProps> = (props) => {
  const { component, position, parentSlot, parentRegion, slotOwnerComponent } = props;
  const layout = useAppSelector(selectLayout);
  const [draggedItem, setDraggedItem] = useState('');
  const componentName = useGetComponentName(component);

  // Check if the drop is allowed based on slot's allowedComponents
  const isDropAllowed = useIsDropAllowed(parentSlot, slotOwnerComponent?.type);
  ...
  const {
    ...
    disabled: draggedItem === `${component.uuid}` || !isDropAllowed,
    data: {
      component: slotOwnerComponent || component,
