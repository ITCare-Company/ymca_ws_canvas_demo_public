...
const DragEventsHandler: React.FC = () => {
  ...
  function handleDragOver(event: DragOverEvent) {
    ...
    const parentComponent = event.over?.data?.current?.component;

    if (parentRegion) {
      dispatch(setTargetSlot(parentRegion.id));
      dispatch(setTargetSlotInfo(undefined)); // Regions don't have allowedComponents
    } else if (parentSlot) {
      dispatch(setTargetSlot(parentSlot.id));
      // Store the parent component type to enable looking up allowedComponents
      dispatch(
        setTargetSlotInfo({
          slotId: parentSlot.name || parentSlot.id,
